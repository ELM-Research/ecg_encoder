...
def get_args(mode: Mode) -> argparse.Namespace:
    ...
    if "train" in mode:
        ...
        # Muon-specific arguments (only used when --optimizer muon)
        parser.add_argument("--muon_momentum", type=float, default=0.95, help="Muon momentum")
        parser.add_argument("--muon_nesterov", action="store_true", default=True, help="Use Nesterov momentum for Muon")
        parser.add_argument("--muon_ns_steps", type=int, default=5, help="Newton-Schulz iteration steps")
        parser.add_argument("--muon_adamw_lr_ratio", type=float, default=0.1, help="AdamW LR as fraction of Muon LR")
